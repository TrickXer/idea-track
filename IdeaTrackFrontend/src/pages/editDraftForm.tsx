...
const EditDraftForm: React.FC = () => {
  ...
  const update = <K extends keyof IdeaCreateRequest>(key: K, value: IdeaCreateRequest[K]) => {
    setForm((prev: IdeaCreateRequest) => ({ ...prev, [key]: value }));
  };
  ...
  const handleAction = async (e: React.FormEvent, actionType: 'save' | 'submit') => {
    ...
    try {
      ...
      if (actionType === 'save') {
        ...
        // For REFINE ideas, resubmit immediately after saving so the backend
        // resets stale reviewer decisions and moves the idea back to SUBMITTED.
        if (ideaStatus?.toUpperCase() === 'REFINE') {
          await submitFinalIdea(ideaIdNum);
          toast.success('Idea updated and resubmitted for review!');
        } else {
      ...
      <div className="card border-0 shadow-sm rounded-4 mx-auto" style={{ maxWidth: '800px' }}>
        <div className="card-body p-4">
          <h4 className="fw-bold mb-4">
            {ideaStatus?.toUpperCase() === 'REFINE' ? 'Refine Your Idea' : 'Edit Your Draft'}
          </h4>
          {ideaStatus?.toUpperCase() === 'REFINE' && (
            <div className="alert alert-warning py-2 mb-4" style={{ fontSize: '.87rem' }}>
              <i className="bi bi-arrow-repeat me-2"></i>
              This idea was sent back for refinement. Update it and save — it will remain in <strong>REFINE</strong> state until a reviewer progresses it.
            </div>
          )}
          ...
          <form noValidate>
            ...
            <div className="d-flex justify-content-end gap-2 pt-4 border-top">
              ...
              >
                {submitting
                  ? (ideaStatus?.toUpperCase() === 'REFINE' ? 'Resubmitting...' : 'Saving...')
                  : (ideaStatus?.toUpperCase() === 'REFINE' ? 'Save & Resubmit' : 'Save Changes')}
              </button>
